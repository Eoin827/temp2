...
class WaveNet(nn.Module):
    def __init__(
        self, nLayers, nStacks, nChannels, nResChannels, nSkipChannels, numOutputLayers
    ):
        ...
        self.conv0 = nn.Conv1d(
            in_channels=1,
            out_channels=self.nResChannels,
            kernel_size=1,
            stride=1,
            padding=0,
            dilation=1,
        ...
        for iLayers in range(self.nLayers * self.nStacks):
            ...
            dilation = 2**expval
            self.residualBlockList.append(
                WaveNetUnit(self.nResChannels, self.nSkipChannels, dilation)
            )
        ...
        for oLayers in range(self.numOutputLayers):
            ...
            if oLayers == self.numOutputLayers - 1:
                self.outputLayers.append(
                    nn.Conv1d(
                        in_channels=self.nSkipChannels,
                ...
                self.outputLayers.append(
                    nn.Conv1d(
                        in_channels=self.nSkipChannels,
                        out_channels=self.nSkipChannels,
                        kernel_size=1,
                    )
                )
...
class WaveNetUnit(nn.Module):
    def __init__(self, nResChannels, nSkipChannels, filter_dilation):
        ...
        self.conv_filter = nn.Conv1d(
            in_channels=nResChannels,
            out_channels=nResChannels,
            kernel_size=2,
            stride=1,
            padding=0,
        ...
        self.conv1x1_res = nn.Conv1d(
            in_channels=nResChannels, out_channels=nResChannels, kernel_size=1
        )
        self.conv1x1_skip = nn.Conv1d(
            in_channels=nResChannels, out_channels=nSkipChannels, kernel_size=1
        )
